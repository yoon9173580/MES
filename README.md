...
## Backtest Results (v10.6 — Real Databento CME Data)
...
> |---|--:|--:|--:|--:|--:|
> | 2023 | 67 | 50.7% | 41.4% | 1.63 | 6.7% |
> | 2024 | 68 | 55.9% | 37.9% | 1.59 | 7.1% |
> | 2025 | 89 | 65.2% | 299% ⚠️ | 4.35 | 5.9% |
> | 2026 (partial) | 33 | 51.5% | 157% ⚠️ | 3.10 | 7.2% |
>
> **Why 0.43→0.35 is considered safe:** the threshold sweep showed Sharpe
> improvement in **all 4 years simultaneously** — the trades ML "barely
> rejected" (P(win) 0.35–0.43) were uncertain-but-valid, not garbage.
> Universal year improvement is the strongest robustness signal available
> on this dataset, though it remains in-sample tuning.
>
...
v10.1 결과 (score≥88): 2거래, 자본 보존. v10.6 (score≥65 + TREND_BEAR)에서는
더 많은 SHORT 진입 예상.
...
|---|---|---|
| **Max Drawdown** | 6.9% (운 좋은 순서) | 중앙값 **9.6%** · p95 **14.7%** · p99 ~18% |
| **총수익** | +511% | p50 +503% · p5 +241% · 손실확률 **0%** (bootstrap) |
| **회복 기간**(신고가까지) | — | 중앙값 **16거래(~2.5개월)** · p95 30거래(~5개월) · 최악 60거래(~9개월) |

**핵심 두 가지 (혼동 주의):**
- **수익률 위치는 전형적(52nd pct, bootstrap)** 이지만, **드로다운은 운이 좋았다(5~9th pct).** →
  라이브 DD는 6.9%가 아니라 **중앙값 9.6%, 대비는 p95 14.7%**로 계획할 것.
  P(MaxDD > 15%) ≈ 4–7%, P(MaxDD > 20%) < 1%.
- **회복은 느리다.** 손실 후 신고가 회복까지 보통 ~2.5개월, 나쁘면 5~9개월.
  v10.6에서 빈도가 55거래/년으로 늘어 회복 *거래 수*는 비슷해도 *달력 기간*은 단축됐다.
...
- **유일한 진짜 레버는 거래 빈도↑** (회복 winner를 더 자주 만남). MIN_SCORE를 더 낮추면
  Sharpe가 하락하지만 (검증됨), **ML SKIP_THRESH 0.43→0.35 완화는 전 연도 Sharpe 개선**과
  함께 빈도를 31→55거래/년으로 올렸다 — v10.6에서 채택.

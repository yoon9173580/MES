...
- **전략:** 하루 1회, 미국장 **10:30 ET(PRIME) 단일 진입**. 7-레이어 스코어가 **74점 이상**일 때만 진입. EOD(15:30 ET) 청산. 저빈도·고확신.
- **현재 성과(백테스트, 2023-03~2026-05 전체, RTH, $10k 시작):** 승률 56.6%, 연수익 31.8%, Sharpe 1.57, MaxDD 5.8%, 129거래.
  - ⚠️ **과최적화·레버리지 주의:** `MIN_SCORE=74`·`SL_CAP=22`는 이 데이터로 그리드서치했고, 헤드라인은 2.5% 리스크(레버리지). **인샘플 최적화 수치**라 라이브 기대치는 더 낮다. walk-forward OOS는 테스트 연도 Sharpe 1.1~1.6(전 연도 흑자). 보수적 구버전(score88·1.5%·SLcap15)은 같은 데이터에서 **8.8%/Sharpe 0.46**. (§3, §8 참조)
...
- **`RISK_PCT` 1.5→2.5% (레버리지 선택):** 위에서 DD 여유가 생겨 사이즈 업 → 연수익 ↑. 단 이건 수익·위험을 비례 확대하는 노브(리스크 선호도 선택). 2.0%면 연수익~20%/DD~4%, 2.5%면 ~31.6%/DD 5.8%.
- **`MIN_SCORE` 60→74 (선택성):** 그리드서치 정점. 한계 거래를 걸러 승률·Sharpe↑. 더 높이면 거래수가 급감해 연수익↓.
- **`VIX_THRESHOLD` 20→25:** 2023-26 강세장에서 VIX 20-25 구간의 평균회귀(역추세 SHORT)가 승률 21%로 독이었음 → 추세추종 구간으로 흡수.

> ⚠️ **과최적화 경고:** `SL_CAP=22`·`MIN_SCORE=74`는 **결과를 보고하는 바로 그 2023-26 데이터로 그리드서치**한 값이다(인샘플 최적화). `RISK 2.5%`는 순수 레버리지(수익·위험 동반 확대). 따라서 31.8%/Sharpe 1.57은 **낙관적 상한**으로 봐야 한다. 정직한 강건성은 walk-forward로 본다:
>
...
| **v10.3** | **SL_CAP 15→22, RISK_PCT 1.5→2.5%** | **57%/31.6%/1.56** |
...
- 머지 완료: PR #17(main `c7e538d`).
- `api/data.py`의 `bear_market_2022` 요약은 `status: PENDING_REBACKTEST` (2022 Databento 데이터로 재백테스트 필요 — 미완).

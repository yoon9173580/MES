...
- **전략:** 하루 1회, 미국장 **10:30 ET(PRIME) 단일 진입**. 7-레이어 스코어가 **65점 이상**일 때만 진입. EOD(15:30 ET) 청산. 저빈도·고확신.
- **현재 성과(백테스트, 2023-03~2026-05 전체, RTH, $10k 시작):** 승률 51.6%, 연수익 24.5%, Sharpe 1.16, MaxDD 6.8%, 161거래(~52/yr).
  - ⚠️ **과최적화·레버리지 주의:** `MIN_SCORE=65`·`SL_CAP=22`는 이 데이터로 그리드서치했고, 헤드라인은 2.5% 리스크(레버리지). **인샘플 최적화 수치**라 라이브 기대치는 더 낮다. walk-forward OOS는 테스트 연도 Sharpe 1.1~1.6(전 연도 흑자). 보수적 구버전(score88·1.5%·SLcap15)은 같은 데이터에서 **8.8%/Sharpe 0.46**. (§3, §8 참조)
...
2. 7-레이어 스코어 ≥ **MIN_SCORE = 65** 이어야 진입.
...
MIN_SCORE       = 65       # 진입 스코어 임계값  ★ (v10.5: 68→65, +8% 거래빈도, 효율적 프런티어)
...
- **`RISK_PCT` 1.5→2.5% (레버리지 선택):** 위에서 DD 여유가 생겨 사이즈 업 → 연수익 ↑. 단 이건 수익·위험을 비례 확대하는 노브(리스크 선호도 선택). 2.0%면 연수익~16%/DD~5%, 2.5%면 ~24.5%/DD 6.8% (v10.5 score65 기준).
- **`MIN_SCORE` 74→68→65 (거래빈도 개선):** v10.4에서 74→68(연수익 동일 31.6%, +18% 거래). v10.5에서 68→65로 한 단계 더 — 점수 스윕(68/67/66/65/64) 결과 **65가 Sharpe 1.0 붕괴 직전 마지막 안전 계단**(64에서 0.99로 절벽 추락). 65에서 거래 +8%(49→52/yr)이면서 **스윕 중 최저 DD(6.8%)**·maxConsecLoss 5→3. 비용은 CAGR 29.8→24.5%. 추가 거래는 한계 신호라 승률 53.6→51.6%↓, Sharpe 1.42→1.16↓ — 효율 손실을 감수하고 빈도를 택한 의도적 트레이드오프.
- **`VIX_THRESHOLD` 20→25:** 2023-26 강세장에서 VIX 20-25 구간의 평균회귀(역추세 SHORT)가 승률 21%로 독이었음 → 추세추종 구간으로 흡수.

> ⚠️ **과최적화 경고:** `SL_CAP=22`·`MIN_SCORE=65`은 **결과를 보고하는 바로 그 2023-26 데이터로 그리드서치**한 값이다(인샘플 최적화). `RISK 2.5%`는 순수 레버리지(수익·위험 동반 확대). 따라서 24.5%/Sharpe 1.16는 **낙관적 상한**으로 봐야 한다. 정직한 강건성은 walk-forward로 본다:
>
...
| **v10.5** | **MIN_SCORE 68→65 (+8% 거래빈도, 49→52/yr, 효율적 프런티어·DD↓)** | **52%/24.5%/1.16** |
